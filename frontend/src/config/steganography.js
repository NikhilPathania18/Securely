...
const util = {
  ...
  const steg = (function () {
    ...
    Cover.prototype.encode =async function (message, image, options) {
      ...
      var modifiedImageData = new ImageData(data, shadowCanvas.width, shadowCanvas.height);

  // Update the canvas with the new ImageData
  shadowCtx.putImageData(modifiedImageData, 0, 0);
    ...
    Cover.prototype.decode = function (image, options) {
      ...
      if (threshold === 1) {
        ...
        // Implementation for threshold > 1 (currently commented out)
      }
